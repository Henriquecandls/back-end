...
const id=Number(req.params.id)
const pessoas=pessoas.filter(p=>p.id==id);
const{firstname,lastname,gender,age,profession}=req.body;
   if (!firstname || !lastname || !gender || !age || !profession) {
      res.status(400).send("Erro: O pedido não está no formato correto");
    }
    if(perssoa==undefined)

      else
    pessoas.firstname=firstname;
    pessoas.lastname=lastname;
    pessoas.gender=gender;
    pessoas.age=age;
    pessoas.profession=profession
